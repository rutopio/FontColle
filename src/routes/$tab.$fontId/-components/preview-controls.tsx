...
// The preview controls, formerly the detail sidebar. Still a narrow vertical
// stack, because it still renders in a narrow place: its own column at the
// right of the page body on desktop, and the mobile drawer below that. It sits
// inside the body rather than in the layout's sidebar slot so that switching
// to a tab without controls doesn't animate the whole shell open and shut.
export function PreviewControls({
  ...
  // Only keys the transition, so switching tabs replays the fade-and-rise
  ...
  return (
    // Same fade-and-rise the list's FilterSidebar plays when the rail switches
    // panels, keyed on the tab so a Tester <-> Instances switch animates.
    <motion.div
      key={panelKey}
              ...
              <div key={a.tag} className="flex flex-col gap-1.5">
                <div className="flex items-baseline justify-between gap-2">
                  {/* The raw four-letter tag sits in an outline badge so it
                      reads as the axis's code rather than part of its name. */}
                ...
                <input
                  ...
                  step={0.5}
                  onChange={(e) => onAxisChange(a.tag, Number(e.target.value))}
          ...
          <h2 className="flex items-center gap-1.5 font-medium text-primary text-sm uppercase">
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs normal-case">
                Each toggle starts in the state browsers apply by default per
                the W3C CSS Fonts spec: features like liga, calt, kern and ccmp
                are on, the rest off.
              </TooltipContent>
            </Tooltip>
          </h2>
