...
// The "fonts with a similar x-height" range each row links to.
...
// The family's own glyphs with guides at the ratios it declares, so the
// numbers below have a picture. A guide with no value is hidden.
function MetricsDiagram({
  ...
  // The specimen is drawn at fontSize 100, so getComputedTextLength already
  // returns diagram units.
  ...
  // Prefer the OS/2 typo* trio: those track the design's ascender, sitting just
  // above cap-height. hhea* is a line-layout metric padded for accents and CJK,
  // so it draws the ascender line far too high. Falls back to hhea*, then
  // cap-height. Descender is positive, for drawing below the baseline.
  ...
  // y grows down, so the baseline sits at ascentTop and a ratio r maps to
  // ascentTop - r.
  const ascTop = asc ?? 1;
  ...
  // The real ink bounds (yMax/yMin), which can exceed the ascender lines.
  const winAR = upm && font.winAscent != null ? font.winAscent / upm : null;
  const winDR = upm && font.winDescent != null ? font.winDescent / upm : null;

  // The win bounds go left, to set them apart from the typo/cap/x guides.
  ...
  const guides: Guide[] = [
    ...
    asc != null
      ? {
          r: asc,
          // Name the source, or the number reads as the letter ascender.
          label:
  ...
  const padL = 120;
  const glyphX = padL;
  // The specimen's width varies hugely by family (~420 units in Montserrat,
  // ~900 in a wide display face), so a fixed budget either wastes space or lets
  // the glyphs lap the labels. Hence the measurement above, with this as the
  // fallback until it lands on SSR / first paint.
...
// Renders nothing when the font carries no usable metric at all.
