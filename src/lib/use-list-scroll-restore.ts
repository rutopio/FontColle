...
// returning from a detail page. The document (window) is the scroll root, so
// this tracks window.scrollY. Two obstacles the naive one-shot restore hits:
// the virtualizer grows its total height over the first frames (so the target
// isn't reachable immediately), and it resets scroll as it mounts/measures (so
// a one-shot restore gets clobbered). So we keep re-asserting the target across
// a short time budget, stopping only once the budget elapses — not the first
// time it's reached.
export function useListScrollRestore(listScrollY: React.RefObject<number>) {
  ...
  useEffect(() => {
    ...
    // Re-assert the target for a fixed budget: the virtualizer resets scroll a
    // few times as it mounts and measures, so a "stop once reached" restore
