...
const shapeMap: Record<ShapeVariant, ShapeClasses> = {
  ...
  rounded: {
    // Rows use the small radius the pre-migration select had; the popup and
    // the trigger keep --radius, matching every other surface in the app.
    item: "rounded-sm",
    bg: "rounded-sm",
...
function useShape(): ShapeClasses {
  ...
  // No ShapeProvider is mounted in this app, so this fallback is what every
  // consumer actually renders. The registry ships "pill" here, whose 20px
  // radius capsules a 36px row; "rounded" tracks --radius like the rest of
  // the UI. Change this, not the call sites.
