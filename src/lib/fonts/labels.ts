...
// readable names without shipping gftools.
...
// These strings are both the bucket keys and the UI headings, and must match
...
/** "PYRS" -> "PYRS Fontlab"; unregistered codes pass through unchanged. */
...
/** Several for a widely spoken language: English lands under every continent,
 *  as it does on Google Fonts. */
function languageRegions(id: string): string[] {
  const r = languages[id]?.regions;
  return r && r.length > 0 ? r : [NO_LANGUAGE_REGION];
}

/** Many-to-many: an item spoken on several continents is listed under each,
 *  so the region counts deliberately sum to more than items.length. */
...
/** `topNSet` ranks by speaker population, never by the incoming order, so
 *  flipping to A–Z reorders pills without changing which ones hide. */
...
// Summed over every language written in the script, so Writing system pills
// rank by real-world reach instead of how many fonts declare that subset.
