...
export function SelectPopup({
    ...
    sideOffset = 4,
    align = "start",
    // Shift the popup left by the item's check column + left padding so the
    // item text lands under the trigger's value text (see min-w widen above).
    alignOffset = -22,
        ...
        <SelectPrimitive.Portal {...portalProps}>
            ...
            >
                ...
                >
                    ...
                    {/* Widen the popup past the anchor by the item's check
                        column so the checkmark overhangs to the left of the
                        trigger's value text (base-ui 1.6.0 doesn't emit the
                        data-side="none" this used to key off of, so widen
                        unconditionally). Pair with alignOffset below to line the
                        item text up under the trigger value. */}
                    <div className="relative h-full min-w-[calc(var(--anchor-width)+1.75rem)] rounded-lg border bg-popover not-dark:bg-clip-padding shadow-lg/5 before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]">
